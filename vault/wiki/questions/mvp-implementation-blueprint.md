...
question: "Rethought from first principles: MVP implementation plan where spec-hardening and harness layers are markdown-based skills, only drift monitoring and event bus remain as code."
...
**New plan (v2)**: 4 TypeScript files (event bus, drift monitor, types, config). Six markdown-based skills for pipeline layers. Progressive disclosure keeps context lean. Zero-compile iteration for harness behavior changes.

**Core insight**: The harness is NOT a code pipeline — it's a skill coordination layer. The model is better at evaluation (spec quality, plan correctness, code review) than imperative code. Code is for determinism: the event bus MUST fire on every event, the drift monitor MUST pattern-match on every `tool_result`. Everything else is probabilistic evaluation and SHOULD be a skill.
...
|-----------|---------------|-----------------|
| **F0** | CODE | Event bus, types, config, extension wiring |
...
├── events.ts               # Event bus: wires pi's 5 native events → skill invocations + drift monitor
...
.pi/
├── extensions/
│   └── harness-event-bus.ts    # Pi extension: wires src/harness/ to pi hooks
├── harness/
...
**Key rule**: `src/harness/` modules are pure TypeScript. Only `events.ts` touches pi's ExtensionAPI. All harness logic lives in skills — markdown, not code.
...
### 2.3 Event Bus Architecture (`src/harness/events.ts`)

The event bus is the **only orchestrator**. It's a thin router: pi native event → determine pipeline phase → invoke appropriate skill or code. No business logic. ~200 lines.

```
Pi Native Event          → Event Bus Action
─────────────────────────────────────────────
session_start            → Load harness config, set state.idle
before_agent_start       → If /harness command detected:
                           → Phase L1: invoke harness-spec skill
                           → Phase L2: invoke harness-plan skill
                           → Phase L4: invoke harness-critic skill + spawn critic agent
                           → Phase P20: invoke harness-gate skill
                           → Phase L5-L8: invoke harness-observe + memory skills
...
**Skill invocation pattern** (pseudocode):

```typescript
// events.ts — invoked when pipeline reaches L1
async function runL1SpecHardening(state: PipelineState): Promise<HardenedSpec> {
  // Event bus injects skill activation context
  const ctx = {
    userRequest: state.spec?.request,
    wikiContext: await queryWiki('spec-hardening'),
    config: state.config.specs,
  };
  // Invoke harness-spec skill — model reads SKILL.md, executes instructions
  // Skill returns structured spec via harness_ask tool or direct generation
  return await invokeSkill('harness-spec', ctx);
}
```
...
**Total MVP**: ~8 weeks (down from ~9 weeks in v1 — skill iteration is faster than code).
...
3. `src/harness/events.ts` — Event bus: routes pi events → skill invocations + drift monitor
4. `src/harness/drift-monitor.ts` — LLM-first drift detection + 6-rule pre-filter + escalation
...
### Config + Wiring
17. `.pi/extensions/harness-event-bus.ts` — Pi extension wiring
18. `.pi/harness/config.json` — Single config file
19. `.pi/agents/critic.md` — L4 critic agent definition
20. `.github/ISSUE_TEMPLATE/harness-spec.yml` — GitHub Issues spec template

**Total**: 4 code files + 12 skill files + 4 config files = 20 files. Down from 15 code files (v1) — shift from code to configuration.
...
|-----|----------|-------------------|
| **ADR-012** | Extension-based harness event bus. No fork. | Event bus is the ONLY extension — invokes skills instead of calling code functions. |
| **ADR-015** | Pipeline-first build order. Validate gates before P43. | Unchanged. Skills deliver same pipeline, same order. |
| **ADR-017** | `src/harness/` library + thin extension wiring. | Now truly thin: 4 files vs 15. Business logic extracted to skills. |
...
|-----------|----------------|-------------------|
| TS source files | 15 | 4 |
| TS lines of code | ~2,500 | ~800 |
| Skill markdown files | 0 | 12 (6 SKILL.md + 6 reference.md) |
| Compilation required per change | Every logic change | Only when types/drift/event bus change |
...
| Deterministic guarantees | All code (guaranteed) | Code for critical path (events + drift), skills for evaluation (probabilistic) |
