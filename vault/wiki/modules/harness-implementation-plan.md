...
| P30 | Browser Subagent — headless browser control (Puppeteer/Playwright) for visual verification of UI changes. Antigravity browser subagent pattern. Dispatched by P25 subagent router for UI tasks. | L3 tools | `lib/harness-browser.ts`, `extensions/harness-browser.ts`, `.pi/harness/browser.json` |
...
| **Puppeteer/Playwright** (headless browser) | P30 | Drive headless Chromium for visual verification, screenshots, UI testing | `npm install puppeteer`. Subagent dispatched by P25 router for UI tasks. Config: `.pi/harness/browser.json` |
