...
export default function (pi: ExtensionAPI) {
	pi.on("session_start", async (_event, ctx) => {
		...
		if (state.available) {
			ctx.ui.notify(
				`lean-ctx overrides active (${state.bin ?? "lean-ctx"}). Built-in tools replaced.`,
				"info",
			);
		} else {
			ctx.ui.notify(
				"lean-ctx not found. Default tools allowed until installed.",
				"warning",
			);
		}
	});
	...
	pi.registerCommand("lean-ctx-status", {
		...
		handler: async (_args, ctx) => {
			...
			if (state.available) {
				ctx.ui.notify(
					`lean-ctx available: ${state.bin ?? "lean-ctx"}. Built-in tools are overridden.`,
					"info",
				);
			} else {
				ctx.ui.notify(
					"lean-ctx unavailable. Built-in tools act normally.",
					"warning",
				);
			}
		},
	...
	pi.registerTool({
		...
		label: "read",
		description:
			"Read the contents of a file. Uses lean-ctx under the hood for efficient reading.",
				...
				if (result.code !== 0) {
					return {
						content: [
							{
								type: "text",
								text: result.stderr || result.stdout || "Error reading file",
							},
						],
						details: { error: true },
					};
	...
	pi.registerTool({
		...
		label: "bash",
		description:
			"Execute a bash command. Automatically wraps commands with lean-ctx for efficient output.",
