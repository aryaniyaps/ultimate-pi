/**
 * harness-subagents — In-house sub-agent extension for ultimate-pi.
 *
 * Tools: Agent, get_subagent_result, steer_subagent, blackboard
 * Events: subagents:* (RPC: subagents:rpc:spawn|ping|stop)
 * Agents load from $UP_PKG/.pi/agents/** with project overrides (path ids: harness/planner).
 */
...
export function createHarnessSubagentsExtension(packageRoot: string) {
	return function harnessSubagents(pi: ExtensionAPI) {
		const blackboard = new Blackboard();
		registerBlackboardTool(pi, blackboard);
		...
		/** Reload agents from package + project paths (called on init and each Agent invocation). */
		const reloadCustomAgents = () => {
			const userAgents = loadCustomAgents(process.cwd(), packageRoot);
		...
		const manager = new AgentManager(
			(record) => {
				...
				if (isError) {
					...
					pi.appendEntry("harness-observation", {
						schema_version: "1.0.0",
						contract_version: "1.0.0",
						source: "harness-subagents",
						kind: "trace",
						...
						payload: {
							...
							result_preview: record.result?.slice(0, 500),
						},
					});
					if (record.result?.trim()) {
						blackboard.post(
							`subagent:${record.id}:result`,
							record.result,
							record.id,
							record.type,
							{ summary: record.description, category: "result" },
						);
						pi.appendEntry("subagent:blackboard", {
							key: `subagent:${record.id}:result`,
							value: record.result,
							agentId: record.id,
		...
		pi.on("session_start", async (_event, ctx) => {
			...
			const drift = getDriftReport(packageRoot);
			if (!drift.ok && ctx.hasUI) {
				const summary =
					drift.items.length === 0
						? "agents.manifest.json missing or unreadable"
						: `${drift.items.length} harness agent(s) drift from package manifest (pi update ultimate-pi?)`;
			...
			for (const entry of entries) {
				if (
					entry.type !== "custom" ||
					entry.customType !== "subagent:blackboard"
				)
					continue;
				const d = entry.data;
...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).${scheduleGuideline}`,
				parameters: Type.Object({
					...
					context: Type.Optional(
						Type.Object(
							...
							{
								description:
									"Blackboard context to inject into the subagent system prompt (capped ~8k chars).",
							},
						),
					),
					...scheduleParam,
				...
				renderResult(result, { expanded, isPartial }, theme) {
					...
					const stats = (d: AgentDetails) => {
						...
						if (d.toolUses > 0)
							parts.push(
								`${d.toolUses} tool use${d.toolUses === 1 ? "" : "s"}`,
							);
				...
				execute: async (toolCallId, params, signal, onUpdate, ctx) => {
					...
					const effectiveMaxTurns = normalizeMaxTurns(
						resolvedConfig.maxTurns ??
							getDefaultMaxTurns() ??
							customConfig?.maxTurns ??
							30,
					);
					const contextSpec = params.context as
						| { agent_id?: string; agent_name?: string; keys?: string[] }
						| undefined;
					const systemPromptAppendix = buildBlackboardContextInjection(
						blackboard,
						{
							agentId: contextSpec?.agent_id,
							agentName: contextSpec?.agent_name,
							keys: contextSpec?.keys,
						},
					);
...
export default createHarnessSubagentsExtension(process.cwd());
